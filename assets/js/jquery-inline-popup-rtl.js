...
(function($) {
  ...
  $.fn.inlinePopup = function(options) {
    ...
    this.setDataRow = function() {
		var rt = ($(window).width() - ($(ds).find(inlinePopup.settings.itemSelector).filter(":visible").offset().left + $(ds).find(inlinePopup.settings.itemSelector).filter(":visible").outerWidth()));
      var firstRight = rt,
          currentRow = 0,
          activeElem = "",
          prevRight = firstRight;
      $(ds).find("."+inlinePopup.settings.ipclass).hide();
      $(ds).find(inlinePopup.settings.itemSelector).filter(":visible").each(function() {
        var cur  = $(this), 
        curRight  = ($(window).width() - (cur.offset().left+cur.outerWidth())) ;
        if(curRight <= prevRight) currentRow += 1;
        prevRight = curRight;
