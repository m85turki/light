...
(function($) { // Hide scope, no $ conflict
 ...
 $(selector).calendarsPicker({minDate: 0, maxDate: '+1m +1w'}) */
	$.JQPlugin.createPlugin({
		...
		defaultRenderer: {
			...
			'{popup:start}<div class="calendars-ctrl">{link:clear}{link:change}{link:close}</div>{popup:end}' +
		...
		/** Command actions that may be added to a layout by name.
			<ul>
			<li>pgv - Show the previous month (based on <code>monthsToStep</code> option) - <em>PageUp</em></li>
		...
		commands: {
			...
			change: {
			    text: 'changeText', status: 'changeStatus', // Clear the datepicker
			    keystroke: { keyCode: 45, ctrlKey: true }, // Ctrl + End
			    enabled: function (inst) { return true; },
			    date: function (inst) { return null; },
			    action: function (inst) {
			        
			        __CalendarChangeType(inst);
			        //plugin.change(this);
			    }
			},
	...
	alert('Now in ' + month + '/' + year);
 	...
 	alert('Selected ' + dates);
		...
		_attachments: function(elem, inst) {
			...
			inst.trigger = (!trigger ? $([]) :
				$(trigger).clone().removeAttr('id').addClass(this._triggerClass)
					[inst.options.isRTL ? 'insertAfter' : 'insertAfter'](elem).
					click(function() {
		...
		change: function (elem, immediate) {
 ...
 $(selector).datepick('setDate', [date1, date2, date3]) */
		...
		_generateMonthSelection: function(inst, year, month, minDate, maxDate, monthHeader, calendar) {
			...
			var html = monthHeader.replace(/m+/i, '\\x2E').replace(/y+/i, '\\x2F').replace(/g+/i,'\\x2G');
